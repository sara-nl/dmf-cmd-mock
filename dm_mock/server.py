...
class DmServer(Server):
    ...
    def __init__(self, socket_file, **kwargs):
        kwargs['tick_sec'] = 10
        super(DmServer, self).__init__(DmServer.get_socket_file(),
                                       **kwargs)
        self.inodes = {}
        dirname = DmServer.get_dm_data_dir()
    ...
    def read_data(self):
        for root, dirs, files in os.walk(DmServer.get_dm_data_dir()):
            for f in files:
                if f.endswith(".json"):
    ...
    def ls_inode(self, inode):
        data_path = os.path.join(DmServer.get_dm_data_dir(),
                                 "%d.json" % inode)
